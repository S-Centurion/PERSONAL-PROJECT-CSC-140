...
void Adef1(string input)
{
	...
	if (compareStr(num, "-1"))
	{
		...
		else if (compareStr(object, "picture")) 
		{ 
...
void Adef2(string input)
{
	...
	if (compareStr(num, "-1"))
	{
		...
		if (object == d) { fDisp("-(Can't investigate that...)\n\n\n"); }
		else { fDisp("-(Can't investigate that...)\n\n\n"); }
	}
	else if (compareStr(num, "-2"))
	{
...
void Acmdreply(string input, int frame) //needed because def(input) will NOT go directly into cout statement
{
...
int Agetcmdline(int frame, string item)
{
